...
if (isset($_POST['submit'])) {
  ...
  elseif ($ptype=="zp") {
    ...
    header("location:editprofile.php?error=".$ptype."");
  }
  $query="update admin,login set admin.privilege='".$privilege."',login.privilege='".$privilege."',admin.pcode='".$pcode."',admin.desig='".$desig."',admin.name='".$name."',admin.gender='".$gender."',admin.email='".$email."',admin.phone='".$phone."',admin.address='".$address."' where admin.id like '".$id."'";
...
<html lang="en">
<head>
  <title>Gram Sampark | ZP Edit Profile</title>
</head>
<style type="text/css">
        ...
        <div class="form-group">
          ...
          <div class="col-sm-9">
            <select name="ptype" class="form-control" id="ptype" required="" style="background-color: transparent;">
              ...
              <option value="gp" <?php if ($privilege==1) {
                echo "selected='selected'";
              } ?> >Gram Panchayat</option>
              <option value="tp" <?php if ($privilege==2) {
                echo "selected='selected'";
              } ?> >Taluk Panchayat</option>
              <option value="zp" <?php if ($privilege==3) {
                echo "selected='selected'";
              } ?> >Zilla Panchayat</option>
            </select>
          </div>
        </div><br>
        <div class="form-group">
